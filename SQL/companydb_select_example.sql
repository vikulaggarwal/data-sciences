...
select * from works_on order by pno asc, hours desc;
...
select dno, sex, round(avg(salary),2)
from employee
...
select dno, count(*) as 'No of Males' from employee where sex = 'M' group by dno, sex;

-- 23. Display the average, minimum, maximum hours spent in each project

select pno, round(max(hours),2), round(avg(hours),2), round(min(hours),2)
from works_on
group by pno;
...
select dname, round(avg(salary),2) from employee inner join department on dno=dnumber where salary >= 30000 group by dname order by dname;
...
select essn, sum(hours) from works_on group by essn having sum(hours) >= 20 ;

-- 32. What is the average salary of those employees who have more than one
--     dependent

select avg(salary) from employee where ssn in (select essn from (select essn,count(*) from dependent group by essn having count(*) > 1) temp);
...
select d.dname, p.pname from project p inner join department d where p.dnum = d.dnumber;
